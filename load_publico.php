...
$orderCol = isset($_POST['ordenarPor']) && in_array($_POST['ordenarPor'], ['palabra_clave', 'diario_nombre', 'fecha', 'link']) ? $_POST['ordenarPor'] : 'fecha';
...
if ($resultado->num_rows > 0) {
    while ($row = $resultado->fetch_assoc()) {
        ...
        $output['data'] .= '<td><a href="' . htmlspecialchars($row['link']) . '" target="_blank" class="btn btn-sm btn-outline-primary">Ver enlace</a></td>';
        $output['data'] .= '</tr>';
    }
} else {
    $output['data'] .= '<tr><td colspan="4" class="text-center text-muted">No se encontraron resultados para los filtros aplicados</td></tr>';
}

$totalPaginas = ceil($totalFiltro / $limit);
if ($totalPaginas > 1) {
    $output['paginacion'] .= '<nav><ul class="pagination justify-content-center">';
    if ($pagina > 1) {
